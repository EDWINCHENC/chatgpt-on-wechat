...
def build_search_url(searchTerms, count=None, startIndex=None, language=None, cx=None, hq=None, dateRestrict=None,
                     key=None):
    params = {
        "q": searchTerms,
        "num": count,
...
def search_google(model, search_terms, count, api_key, cx_id, iterations):
    all_summaries = []
    ...
    for i in range(iterations):
        ...
        while retries < MAX_RETRIES:
            try:
                ...
                search_url = build_search_url(search_terms, count=10, cx=cx_id, key=api_key,
