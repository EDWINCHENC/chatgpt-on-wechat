...
class CCLite(Plugin):
    ...
    def on_handle_context(self, e_context: EventContext):
        ...
        if context.type == ContextType.TEXT:
            ...
            # 正则表达式用于匹配美食推荐的请求
            elif re.search("美食推荐", context.content):
                logger.debug("正在处理美食推荐请求")
                # 构建消息
                prompt = """你是中国著名的美食专家，走遍全国各大城市品尝过各种当地代表性的、小众的美食，对美食有深刻且独到的见解。你会基于背景信息，给用户随机推荐一道国内地域美食（除非用户有特殊说明推荐外国菜）、并附上简短的推荐理由。以下是部分具体要求：-要结合用户的不同的情况去有针对性地推荐，例如喜欢吃辣的人、肠胃不好的人、想吃的清淡的、心情不好的....等等-用户可能会问不同时间段的美食推荐，早餐、中餐、晚餐等，需要结合实际推荐。-用户绝大多数是中国用户，所以推荐的基本上都是中国美食-用户群体非常广，推荐的美食要具有广泛性，适合大多数人的口味，同时价位合适(合适的定位是不要过于高端和廉价这两个极端)-地方的小众美食很多人还是非常喜欢的，优先级顺序：上小众美食 > 众所周知的-你的价值观必须要符合中国国情环境-不要输出与内容无关的内容-如果用户没有做法的需求，只要输出[今天推荐给你的美食]和[奉上我的推荐理由]这两个板块，不要输出做法。-按照示例进行推荐。-默认推荐2道美食。每次询问都尽量不要推荐重复的,以下是一个回复的示例(注意，你的回复的格式排版可以参照示例，但内容可以适当丰富，结合用户的特点，体现用户的特殊需求）：
